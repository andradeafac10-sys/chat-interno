...
export default function Dashboard() {
  ...
  rotinasFiltradas.forEach(r => {
    if (!porResponsavel[r.responsavel]) {
      porResponsavel[r.responsavel] = { total: 0, completadas: 0, nome: r.responsavel, cor: r.cor_responsavel };
    }
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-[13px]" style={{ color: '#667085' }}>Acompanhamento das rotinas recorrentes por equipe e responsável.</p>
            </div>
          ...
          <div className="grid grid-cols-5 gap-3 mb-6">
            <div className="p-4 rounded-lg" style={{ background: 'var(--surface-1)', border: '0.5px solid #E4E8EE' }}>
              <p className="text-[12px] font-semibold uppercase mb-2" style={{ color: '#667085' }}>Total de rotinas</p>
              <p className="text-[28px] font-semibold" style={{ color: '#101828' }}>{totalRotinas}</p>
              <p className="text-[11px] mt-1" style={{ color: '#667085' }}>do período filtrado</p>
            </div>
            ...
            <div className="p-4 rounded-lg" style={{ background: '#EAF3DE', border: '0.5px solid #B8D77D' }}>
              ...
              <p className="text-[11px] mt-1" style={{ color: '#3B6D11' }}>{totalRotinas > 0 ? Math.round((rotinasCompletadas / totalRotinas) * 100) : 0}% de conclusão</p>
            </div>
            ...
            <div className="p-4 rounded-lg" style={{ background: '#E6F1FB', border: '0.5px solid #8AB9E8' }}>
              ...
              <p className="text-[11px] mt-1" style={{ color: '#185FA5' }}>{totalRotinas > 0 ? Math.round((rotinasEmAndamento / totalRotinas) * 100) : 0}% em execução</p>
            </div>
            ...
            <div className="p-4 rounded-lg" style={{ background: '#FCEBEB', border: '0.5px solid #F4A0A0' }}>
              ...
              <p className="text-[11px] mt-1" style={{ color: '#A32D2D' }}>{totalRotinas > 0 ? Math.round((rotinasAtrasadas / totalRotinas) * 100) : 0}% do total</p>
            </div>

            <div className="p-4 rounded-lg" style={{ background: 'var(--surface-1)', border: '0.5px solid #E4E8EE' }}>
              <p className="text-[12px] font-semibold uppercase mb-2" style={{ color: '#667085' }}>Esta semana</p>
              <p className="text-[28px] font-semibold" style={{ color: '#101828' }}>{proximosPrazos.length}</p>
              <p className="text-[11px] mt-1" style={{ color: '#667085' }}>próximos prazos</p>
            </div>
          </div>
          ...
          {/* View toggle e tabela */}
          <div className="mb-4 flex gap-2">
            <button 
              onClick={() => setView('lista')}
              className={`px-3 py-2 rounded text-[13px] flex items-center gap-2 font-semibold ${view === 'lista' ? 'text-white' : ''}`}
              style={{ background: view === 'lista' ? '#2563EB' : 'var(--surface-1)', color: view === 'lista' ? 'white' : '#101828', border: view !== 'lista' ? '0.5px solid #E4E8EE' : 'none' }}
            >
              <List size={14} />
              Lista
            </button>
            <button 
              ...
              className={`px-3 py-2 rounded text-[13px] flex items-center gap-2 font-semibold ${view === 'kanban' ? 'text-white' : ''}`}
              style={{ background: view === 'kanban' ? '#2563EB' : 'var(--surface-1)', color: view === 'kanban' ? 'white' : '#101828', border: view !== 'kanban' ? '0.5px solid #E4E8EE' : 'none' }}
            >
              <LayoutGrid size={14} />
              Kanban
            </button>
            <button 
              ...
              className={`px-3 py-2 rounded text-[13px] flex items-center gap-2 font-semibold ${view === 'calendario' ? 'text-white' : ''}`}
              style={{ background: view === 'calendario' ? '#2563EB' : 'var(--surface-1)', color: view === 'calendario' ? 'white' : '#101828', border: view !== 'calendario' ? '0.5px solid #E4E8EE' : 'none' }}
            >
              <Calendar size={14} />
              Calendário
            </button>
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr style={{ borderBottom: '0.5px solid #E4E8EE', background: 'var(--pagina-fundo)' }}>
                    ...
                    <th className="px-4 py-3 text-left font-semibold" style={{ color: '#667085' }}>Supervisor</th>
                    ...
                    <th className="px-4 py-3 text-left font-semibold" style={{ color: '#667085' }}>Frequência</th>
                    <th className="px-4 py-3 text-center font-semibold" style={{ color: '#667085' }}>Status</th>
                    <th className="px-4 py-3 text-left font-semibold" style={{ color: '#667085' }}>Obs</th>
                  </tr>
                </thead>
                <tbody>
                  {rotinasFiltradas.slice(0, 10).map((rotina, idx) => (
                    <tr key={rotina.id} style={{ borderBottom: '0.5px solid #E4E8EE' }}>
                      <td className="px-4 py-3" style={{ color: '#101828' }}>{rotina.setor || '-'}</td>
                      <td className="px-4 py-3" style={{ color: '#101828' }}>{rotina.supervisor || '-'}</td>
                      ...
                      <td className="px-4 py-3" style={{ color: '#101828' }}>{rotina.proxima_execucao ? new Date(rotina.proxima_execucao).toLocaleDateString('pt-BR') : '-'}</td>
                      <td className="px-4 py-3">
                        <span className="px-2 py-1 rounded text-[11px] font-semibold" style={{ background: CORES_FREQ[rotina.recurrence_type], color: CORES_FREQ_TEXTO[rotina.recurrence_type] }}>
                          {LABEL_FREQ[rotina.recurrence_type] || rotina.recurrence_type}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span 
                          ...
                          title={LABEL_STATUS[rotina.status] || rotina.status}
                        />
                      </td>
                      <td className="px-4 py-3 text-[12px]" style={{ color: '#667085' }}>{rotina.description ? rotina.description.substring(0, 30) + '...' : '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="p-5 rounded-lg" style={{ background: 'var(--surface-1)', border: '0.5px solid #E4E8EE' }}>
              <h3 className="text-[14px] font-semibold mb-4" style={{ color: '#101828' }}>Status das rotinas</h3>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[14px] font-semibold" style={{ color: '#101828' }}>{rotinasCompletadas} ({totalRotinas > 0 ? Math.round((rotinasCompletadas / totalRotinas) * 100) : 0}%)</span>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="text-[14px] font-semibold" style={{ color: '#101828' }}>{rotinasEmAndamento} ({totalRotinas > 0 ? Math.round((rotinasEmAndamento / totalRotinas) * 100) : 0}%)</span>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="text-[14px] font-semibold" style={{ color: '#101828' }}>{rotinasAtrasadas} ({totalRotinas > 0 ? Math.round((rotinasAtrasadas / totalRotinas) * 100) : 0}%)</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 rounded-lg" style={{ background: 'var(--surface-1)', border: '0.5px solid #E4E8EE' }}>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[14px] font-semibold" style={{ color: '#101828' }}>{porFrequencia.daily}</span>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="text-[14px] font-semibold" style={{ color: '#101828' }}>{porFrequencia.weekly}</span>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="text-[14px] font-semibold" style={{ color: '#101828' }}>{porFrequencia.monthly}</span>
                </div>
              </div>
            </div>

            <div className="p-5 rounded-lg" style={{ background: 'var(--surface-1)', border: '0.5px solid #E4E8EE' }}>
              <h3 className="text-[14px] font-semibold mb-4" style={{ color: '#101828' }}>Próximos prazos (7 dias)</h3>
              <div className="space-y-2 text-[12px]">
                {proximosPrazos.map((r, idx) => (
                  <div key={idx} className="flex justify-between pb-2" style={{ borderBottom: '0.5px solid #E4E8EE' }}>
                    <span style={{ color: '#101828' }}>{new Date(r.proxima_execucao).toLocaleDateString('pt-BR')} - {r.name.substring(0, 20)}</span>
                    <span className="px-1.5 py-0.5 rounded text-[10px]" style={{ background: CORES_FREQ[r.recurrence_type], color: CORES_FREQ_TEXTO[r.recurrence_type] }}>
                      {LABEL_FREQ[r.recurrence_type]}
                    </span>
                  </div>
                ))}
                {proximosPrazos.length === 0 && (
                  <p style={{ color: '#667085' }}>Nenhum prazo próximo</p>
                )}
              </div>
            </div>
          </div>
