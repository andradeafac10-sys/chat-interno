  ...
  return (
    <div className={colapsado ? "w-[64px] h-full shrink-0 flex flex-col overflow-y-auto transition-all" : "w-[210px] h-full shrink-0 flex flex-col overflow-y-auto transition-all"} style={{ background: colors.sidebarBg }}>
      ...
      <button
        ...
        style={{ color: colors.textSecondary, background: colors.sidebarHover }}
      >
        {colapsado ? <ChevronRight size={15} /> : <><ChevronLeft size={14} /> Recolher menu</>}
      </button>

      <div className={colapsado ? "border-t pt-3 pb-1 flex flex-col items-center" : "border-t px-4 pt-3 pb-1"} style={{ borderColor: colors.border }}>
      ...
      <button onClick={onOpenAccount} title={colapsado ? "Configurações" : undefined} className={colapsado ? "flex items-center justify-center py-2 text-[12.5px]" : "flex items-center gap-2.5 px-4 py-2 text-[12.5px]"} style={{ color: colors.textSecondary }}>
        <Settings size={14} /> {!colapsado && "Configurações"}
      </button>
      <button onClick={toggleTheme} title={colapsado ? (theme === "dark" ? "Tema claro" : "Tema escuro") : undefined} className={colapsado ? "flex items-center justify-center py-2 text-[12.5px]" : "flex items-center gap-2.5 px-4 py-2 text-[12.5px]"} style={{ color: colors.textSecondary }}>
