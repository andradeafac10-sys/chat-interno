        ...
        <div className="p-5">
          ...
          {isAdm ? (
            <textarea
              ...
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm mb-4 focus:outline-none focus:ring-2 focus:ring-[#2563EB] resize-none"
            />
          ) : (
          ...
          <div className="flex items-center justify-between mb-1.5">
            ...
            {isAdm && (
              <button onClick={() => attachmentInputRef.current?.click()} className="text-[11px] text-[#2563EB] font-medium flex items-center gap-1">
          ...
          <div className="flex flex-col gap-1 mb-5 max-h-52 overflow-y-auto">
            {(group.attachments || []).map((att) => (
              <div key={att.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-slate-50">
                ...
                <a href={fileUrl(att.file_url)} download={att.file_name} className="text-slate-400 hover:text-[#2563EB] shrink-0"><Download size={13} /></a>
                {isAdm && (
