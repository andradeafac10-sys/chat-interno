...
router.get('/minhas', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT rc.id, rc.occurrence_date, rc.done, rc.done_at, r.id AS recurrence_id, r.title, r.description, r.priority, r.start_time
...
// -------------------------------------------------------------
// GET /api/gestao/recurrences/minhas/atrasadas-count — pro contador vermelho
// no menu lateral (Chat) e na faixa de avisos.
// -------------------------------------------------------------
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT COUNT(*)::int AS total
       FROM routine_completions rc
       JOIN task_recurrences r ON r.id = rc.recurrence_id
       WHERE rc.user_id = $1 AND rc.occurrence_date = $2 AND rc.done = false
         AND r.start_time IS NOT NULL AND r.start_time <= $3`,
      [req.user.id, hoje, horaAgora]
    );
...
// (só a própria pessoa marca a própria rotina). Aceita "nota" (relato) opcional
// e avisa todo mundo quando conclui, igual uma notificação de mensagem.
// -------------------------------------------------------------
...
// POST /api/gestao/recurrences/completions/upload -> sobe um arquivo pra anexar numa rotina do dia
...
// Dados reais de HOJE pra tela Visão Geral/Dashboard: quantos planejados/
// concluídos/atrasados, quais precisam de atenção, quais ainda vêm, e o que
// foi concluído recentemente — mais os KPIs extras de tarefas, alinhamentos
// e treinamentos. Tudo calculado na hora, nada é guardado à parte.
// -------------------------------------------------------------
router.get('/visao-geral-hoje', async (req, res) => {
  try {
    ...
    // KPIs extras — tarefas em aberto, alinhamentos e treinamentos pendentes.
    // Ficam dentro deste mesmo endpoint pra tela ler tudo numa chamada só.
    const filtroUser = assigneeId ? 'AND EXISTS (SELECT 1 FROM task_assignees ta WHERE ta.task_id = t.id AND ta.user_id = $1)' : '';
    const { rows: tarefasRows } = await pool.query(
      `SELECT COUNT(*)::int AS total FROM tasks t
       WHERE t.status NOT IN ('done','canceled') ${filtroUser}`,
      assigneeId ? [assigneeId] : []
    );

    const filtroFeedback = assigneeId
      ? 'WHERE f.id IN (SELECT feedback_id FROM feedback_recipients WHERE user_id = $1)'
      : '';
    const { rows: feedbacksRows } = await pool.query(
      `SELECT
         COUNT(*) FILTER (WHERE NOT EXISTS (
           SELECT 1 FROM feedback_recipients fr WHERE fr.feedback_id = f.id AND fr.acknowledged_at IS NULL
         ))::int AS concluidos,
         COUNT(*) FILTER (WHERE EXISTS (
           SELECT 1 FROM feedback_recipients fr WHERE fr.feedback_id = f.id AND fr.acknowledged_at IS NULL
         ))::int AS pendentes
       FROM feedbacks f ${filtroFeedback}`,
      assigneeId ? [assigneeId] : []
    );

    const filtroTrilha = assigneeId ? 'AND tp.user_id = $1' : '';
    const { rows: trilhaRows } = await pool.query(
      `SELECT
         COUNT(*) FILTER (WHERE tp.concluido_em IS NOT NULL)::int AS concluidos,
         COUNT(*) FILTER (WHERE tp.concluido_em IS NULL)::int AS pendentes
       FROM trilha_progresso tp
       WHERE 1=1 ${filtroTrilha}`,
      assigneeId ? [assigneeId] : []
    );
    ...
    res.json({
      ...
      tarefasPendentes: tarefasRows[0]?.total || 0,
      feedbacksPendentes: feedbacksRows[0]?.pendentes || 0,
      feedbacksConcluidos: feedbacksRows[0]?.concluidos || 0,
      treinamentosPendentes: trilhaRows[0]?.pendentes || 0,
      treinamentosConcluidos: trilhaRows[0]?.concluidos || 0,
    });
  } catch (err) {
...
router.get('/ranking/dados', async (req, res) => {
  try {
    ...
    res.json({ periodo: req.query.periodo || 'custom', ranking });
  } catch (err) {
...
// -------------------------------------------------------------
// Lembretes automáticos: 15 e 5 minutos antes do horário marcado de uma
// rotina ainda não feita. Chamado pelo servidor a cada minuto.
// -------------------------------------------------------------
...
module.exports = { router, gerarTodasAsOcorrencias, gerarOcorrenciasDaRotina, verificarLembretesRotinas };
